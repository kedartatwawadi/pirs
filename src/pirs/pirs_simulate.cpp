...
SimulationFiles::SimulationFiles(const SimulationParameters &params)
{
	...
	const char *clean_read_suffix = ".clean";
	...
	out_file_clean_1.open(prefix_long + "_1" + clean_read_suffix);
	out_file_clean_2.open(prefix_long + "_2" + clean_read_suffix);
 ...
 */
static void output_read(const Read &read, OutputStream &out_file, OutputStream &out_file_clean)
{
	...
	out_file_clean.write(&read.raw_read[0], read.raw_read.size());
	out_file_clean.putc('\n');
				    ...
				    OutputStream &out_file_clean)
{
	...
	do {
		...
		for (size_t i = 0; i < READS_PER_SET; i++) {
			...
			output_read(*set->reads[i], out_file, out_file_clean);
		}
			     ...
			     RandomBitGenerator &bits)
{
	...
	if (bits.next_bit()) {
		...
		output_read(pair.read_1, files.out_file_1, files.out_file_clean_1);
		output_read(pair.read_2, files.out_file_2, files.out_file_clean_2);
	} else {
		pair.reverse_order = true;
		log_read_info(pair.read_2, files.info_log_file);
		log_read_info(pair.read_1, files.info_log_file);
		output_read(pair.read_2, files.out_file_1, files.out_file_clean_1);
		output_read(pair.read_1, files.out_file_2, files.out_file_clean_2);
	}
}
